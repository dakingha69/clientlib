 ...
 */
export var fetchUrl = function (url, options) { return __awaiter(_this, void 0, void 0, function () {
    var response, json;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, (typeof fetch === "function" ? fetch(url, options) : nodeFetch(url, options))];
            case 1:
                response = _a.sent();
 ...
 */
export var websocketStream = function (url, functionName, args, reconnectingOptions) {
    if (reconnectingOptions === void 0) { reconnectingOptions = {}; }
    return Observable.create(function (observer) {
        var options = __assign({ WebSocket: global.WebSocket ? undefined : NodeWebSocket, minReconnectionDelay: 1, reconnectOnError: true }, reconnectingOptions);
 ...
 */
export var formatToDelegationFeesInternal = function (raw, decimals, currencyNetworkOfFees) {
    return {
        decimals: decimals,
        raw: new BigNumber(raw),
        value: calcValue(raw, decimals),
 ...
 */
export var formatEvent = function (event, networkDecimals, interestRateDecimals) {
    ...
    for (var _i = 0, keys_1 = keys; _i < keys_1.length; _i++) {
        var key = keys_1[_i];
        if (event[key]) {
            event[key] = formatToAmount(event[key], key.includes('interestRate') ? interestRateDecimals : networkDecimals);
        }
    }
    return event;
};
 ...
 */
export var formatExchangeEvent = function (exchangeEvent, makerDecimals, takerDecimals) {
    if (exchangeEvent.type === 'LogFill') {
        var fillEventRaw = exchangeEvent;
        return __assign({}, fillEventRaw, { filledMakerAmount: formatToAmount(fillEventRaw.filledMakerAmount, makerDecimals), filledTakerAmount: formatToAmount(fillEventRaw.filledTakerAmount, takerDecimals) });
    }
    else if (exchangeEvent.type === 'LogCancel') {
        var cancelEventRaw = exchangeEvent;
        return __assign({}, cancelEventRaw, { cancelledMakerAmount: formatToAmount(cancelEventRaw.cancelledMakerAmount, makerDecimals), cancelledTakerAmount: formatToAmount(cancelEventRaw.cancelledTakerAmount, takerDecimals) });
    }
